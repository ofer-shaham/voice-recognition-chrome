    ...
    "/api/srt": {
      get: {
        ...
        parameters: [
          ...
          { name: "method", in: "query", required: false, schema: { type: "string", enum: ["1", "2", "3"] }, description: "1=youtube-transcript-plus, 2=youtube-transcript-api-js, 3=OnRender youtube-dl-jrte.com service" },
        ],
...
app.get("/api/transcript/languages", async (req, res) => {
  ...
  try {
    if (service === "onrender") {
      const url = `https://www.youtube.com/watch?v=${encodeURIComponent(String(videoId))}`;
      const upstream = await fetch(`https://youtube-dl-jrte.onrender.com/api/video-info?url=${encodeURIComponent(url)}`);
      const body = await upstream.text();
      let data;
      try { data = JSON.parse(body); } catch { data = null; }
      if (!upstream.ok || data?.error) {
        throw new Error(data?.message || data?.error || `OnRender video-info HTTP ${upstream.status}`);
      }
      const tracks = data?.subtitles || data?.tracks || data?.availableLanguages || data?.languages || [];
      const vd = data?.videoDetails || data?.video || data || {};
      log("info", "OnRender video-info OK", { videoId, n: tracks.length });
      return res.json({
        videoDetails: {
          title: vd.title || data?.title || null, author: vd.author || data?.author || null,
          lengthSeconds: vd.lengthSeconds || vd.duration || null, videoId: vd.videoId || data?.videoId || videoId,
        },
        availableLanguages: tracks.map(t => ({
          languageCode: t.languageCode || t.lang || t.code || t.language || String(t),
          name: t.name || t.label || t.languageName || t.languageCode || t.lang || String(t),
          isAutoGenerated: !!(t.isAutoGenerated || t.autoGenerated || t.kind === "asr"),
        })),
      });
    }
    if (instanceUrl) {
    ...
    if (tracks.length === 0) {
      const fallback = await fetchLanguagesMethod2(String(videoId), proxyUrl);
      log("info", "transcript/languages fallback OK", { videoId, n: fallback.availableLanguages.length });
