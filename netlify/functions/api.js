...
exports.handler = async (event) => {
  ...
  // Keep the client-facing subtitle endpoints available in production too.
  // The local Express server has the richer fallback chain; Netlify delegates
  // the same simple contract to the hosted caption service.
  if (method === "GET" && apiPath === "/api/transcript/languages") {
    ...
    try {
      ...
      const video = data?.videoDetails || data?.video || data || {};
      return json(200, {
        videoDetails: {
          title: video.title || data?.title || null,
          description: video.description || data?.description || null,
          videoId: video.videoId || data?.videoId || videoId,
        },
      ...
      log("error", "transcript/languages proxy failed", { videoId, error: err.message });
  ...
  if (method === "GET" && apiPath === "/api/srt") {
    ...
    try {
      ...
      if (!upstream.ok) throw new Error(`Subtitle service HTTP ${upstream.status}: ${upstream.body.slice(0, 200)}`);
      const parsed = parseJson(upstream.body);
      const content = typeof parsed === "string"
        ? parsed
        : parsed?.content || parsed?.subtitle || parsed?.srt || parsed?.text || parsed?.data?.content;
      const body = typeof content === "string" ? content : upstream.body;
      if (!body.trim()) throw new Error("Subtitle service returned an empty file");
      return textResp(200, body);
    } catch (err) {
      log("error", "srt proxy failed", { videoId, lang, error: err.message });
      return json(502, { error: err.message });
    }
  }
