...
function YoutubeTranscriptParser() {
  ...
  const loadProject = useCallback((project: YtProject) => {
    ...
    // Build unique labels (#1): append (auto) or (from: X) to distinguish tracks
    const labelCounts: Record<string, number> = {};
    for (const t of project.tracks) {
      labelCounts[t.langLabel] = (labelCounts[t.langLabel] ?? 0) + 1;
    }
    const makeUniqueLabel = (track: typeof project.tracks[0]): string => {
      let label = track.langLabel;
      const needsDisambig = (labelCounts[track.langLabel] ?? 0) > 1 || track.isAutoGenerated || track.translatedFrom;
      if (needsDisambig) {
        if (track.isAutoGenerated)   label += " (auto)";
        if (track.translatedFrom)    label += ` (from: ${track.translatedFrom})`;
      }
      return label;
    };

    const extras: ExtraSrtCol[] = project.tracks.slice(1).map((track, i) => ({
      id:          `extra_${i}_${track.lang}`,
      label:       makeUniqueLabel(track),
      lang:        track.lang,
