  ...
  const handleFindLanguages = async (inputUrl = url, requestedService = subtitleService) => {
    ...
    if (inputInvidiousUrl || (requestedService === 'invidious' && invidiousCaptionsUrl.trim())) {
      ...
      try {
        ...
        // Try to fetch video metadata from YouTube API even if using Invidious captions
        let videoTitle = `Invidious video ${invidiousVideoId}`;
        let videoDescription = 'Subtitle imported from Invidious.';
        try {
          const metadataRes = await fetch(`/api/transcript/languages?videoId=${encodeURIComponent(invidiousVideoId)}`);
          if (metadataRes.ok) {
            const metadataBody = await metadataRes.json().catch(() => ({}));
            if (metadataBody.videoDetails?.title) {
              videoTitle = metadataBody.videoDetails.title;
              videoDescription = metadataBody.videoDetails?.description || `YouTube video transcript for ${invidiousVideoId}`;
            }
          }
        } catch {
          // Fall back to default title/description if API call fails
        }
    ...
    try {
      ...
      const usableLangs: AvailableLang[] = langs.length ? langs : LANG_OPTIONS.map(l => ({
        languageCode: l.code,
        name: `${l.label} · language-code request`,
        isAutoGenerated: false,
      }));
      setVideoId(vid);
      // Always use API metadata if available, providing sensible fallbacks
      ...
      const preferredLang = usableLangs.find(lang => lang.isDefault && lang.isAutoGenerated)
        || usableLangs.find(lang => lang.isDefault)
        || usableLangs.find(lang => lang.isAutoGenerated)
        || usableLangs[0];
      setSelectedLangs(new Set([preferredLang.languageCode]));
      setStep('langs');
    } catch (e: any) {
  ...
  const handleFetchSelected = async () => {
    ...
    try {
      ...
      for (const lang of chosen) {
        ...
        const useDirectSubtitleUrl = !!lang.subtitleUrl && subtitleService === 'iframe';
        const trackUrl = useDirectSubtitleUrl
