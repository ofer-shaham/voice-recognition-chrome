  ...
  const handleFetchStart = async () => {
    ...
    for (const l of ordered) {
      ...
      try {
        ...
        // Fingerprint: collapse all whitespace so minor formatting differences don't matter
        const fingerprint = srtContent.replace(/\s+/g, ' ').trim();
        if (seenFingerprints.has(fingerprint)) {
          dupes.push(l.name);
        } else {
          seenFingerprints.add(fingerprint);
          tracks.push({ lang: l.languageCode, label: l.name, isAuto: l.isAutoGenerated, srtContent });
        }
      } catch {
        skipped.push(l.name);
      }
    ...
    if (!tracks.length) {
      setFetchError(`Could not fetch any transcripts. Skipped: ${skipped.join(', ')}`);
      setFetchStep('langs');
      return;
    }
    const notices: string[] = [];
    if (skipped.length) notices.push(`Skipped (error): ${skipped.join(', ')}`);
    if (dupes.length)   notices.push(`Skipped (duplicate SRT): ${dupes.join(', ')}`);
    if (notices.length) setFetchError(notices.join(' · '));
