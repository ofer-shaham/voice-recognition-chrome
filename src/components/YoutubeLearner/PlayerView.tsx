...
import { YtProject, YtTrack, ParsedLine, ProjectConfig, ColSetting } from './types';
...
import { getTranscriptMethod, setTranscriptMethod as persistTranscriptMethod, TranscriptMethod } from '../../services/transcriptMethodConfig';
...
export default function PlayerView({ project, onSave, onNewVideo, onDelete, projects, onSelectProject }: Props) {
  ...
  const [availLangs, setAvailLangs] = useState<SubtitleInfo[]>([]);
  ...
  const fetchAvailLangs = useCallback(async () => {
    ...
    setLangsError('');
    try {
      const data = await fetchVideoInfo(projectRef.current.videoId);
      // Combine manual and autoTranslated, excluding already loaded languages
      const all = [...data.manualSubtitles, ...data.autoTranslatedSubtitles];
      setAvailLangs(all);
    } catch (e: any) {
  ...
  const addLangTrack = useCallback(async (lang: SubtitleInfo) => {
    setAddingLang(lang.code);
    setLangsError('');
    try {
      const srtContent = await fetchSubtitlesSrt(projectRef.current.videoId, lang.code);
      const newTrack: YtTrack = { lang: lang.code, label: lang.name, isAuto: true, srtContent };
      const p = projectRef.current;
      const newTracks = [...p.tracks, newTrack];
      const colId = `track:${lang.code}`;
