...
export default function PlayerView({ project, onSave, onBackHome, onNewVideo, onDelete, projects, onSelectProject, theme, onThemeChange }: Props) {
  ...
  const fetchAvailLangs = useCallback(async () => {
    ...
    setLangsError('');
    try {
      if (projectRef.current.subtitleIndexUrl) {
        const indexUrl = projectRef.current.subtitleIndexUrl;
        const indexResponse = await fetch(`/api/srt-url?url=${encodeURIComponent(indexUrl)}`);
        if (!indexResponse.ok) {
          const j = await indexResponse.json().catch(() => ({}));
          throw new Error(j.error || `HTTP ${indexResponse.status}`);
        }
        ...
        const directLangs = dedupeAvailLangs<AvailableLang>(captions.map((caption: any): AvailableLang => {
          ...
          return {
            ...
            subtitleUrl: caption.url
              ? buildInvidiousSubtitleUrl(indexUrl, String(caption.url))
              : undefined,
          };
        }).filter((lang: AvailableLang) => lang.languageCode && lang.subtitleUrl));
        if (!directLangs.length) throw new Error('The Invidious response did not contain any usable caption URLs.');
        setAvailLangs(directLangs);
        return;
      }
  ...
  const addLangTrack = useCallback(async (lang: AvailableLang) => {
    ...
    setLangsError('');
    try {
      const res = lang.subtitleUrl
        ? await fetch(`/api/srt-url?url=${encodeURIComponent(lang.subtitleUrl)}`)
        : await (async () => {
          ...
          return fetch(`/api/srt?${query.toString()}`);
        })();
      if (!res.ok) { const j = await res.json().catch(() => ({})); throw new Error(j.error || `HTTP ${res.status}`); }
      const srtContent = await res.text();
      if (!srtContent.trim()) throw new Error('The subtitle response was empty.');
      const newTrack: YtTrack = { lang: lang.languageCode, label: lang.name, isAuto: lang.isAutoGenerated, srtContent, subtitleUrl: lang.subtitleUrl };
